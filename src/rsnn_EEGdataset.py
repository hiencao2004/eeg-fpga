...
sys.path.append('/content/drive/MyDrive/EESNN_Project')

from build_cross_input_mit2s_1test import CNN_INPUT_cross_gaussian
...
model_root_path = '/content/drive/MyDrive/EESNN_Project/models_output/'
if not os.path.exists(model_root_path):
    os.makedirs(model_root_path)

log_root_path = '/content/drive/MyDrive/EESNN_Project/logs_output/'
...
# 2. Xóa các handlers cũ (nếu có) để tránh log bị lặp lại
if logger.hasHandlers():
    logger.handlers.clear()

# 3. Tạo file handler (để ghi ra file)
file_handler = logging.FileHandler(log_path)
file_handler.setLevel(logging.INFO)

# 4. Tạo console handler (để in ra màn hình Colab)
...
# 6. Tạo một lớp giả (wrapper) để code cũ "log.logger.info" hoạt động
class LogWrapper:
...
def evaluate(model, eval_iterator, params, epoch_i, run_mode='eval'):
    ...
    return accuracy, precision, recall, specificity, eval_auc, eval_loss, label_list, predict_list
...
def run_train(model, optimizer, use_eval=1):
    ...
    best_model = torch.load(os.path.join(model_root_path, 'pretrain_bestmodel.pkl'), weights_only=False)
...
def run_test_once(best_model):
    ...
    with torch.no_grad():
        for i in range(test_num):
            ...
            test_result_dic = {}
            test_accuracy, test_precision, test_sensitivity, test_specificity, test_auc, _, y_true, y_scores = evaluate(best_model,
                                                                                                                        test_loader,
                                                                                                                        params,
                                                                                                                        run_mode='test',
                                                                                                                        epoch_i=i)
                # === BẮT ĐẦU CODE VẼ ĐỒ THỊ ===

            # 1. Tạo nhãn dự đoán (y_pred) từ xác suất (y_scores)
            y_pred = [1 if p > params.threshold else 0 for p in y_scores]

            # 2. Vẽ MA TRẬN NHẦM LẪN (Confusion Matrix)
            log.logger.info(f'Đang vẽ Ma trận nhầm lẫn cho {patient_name}...')

            cm = confusion_matrix(y_true, y_pred)
            disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=['Non-Seizure', 'Seizure'])
            disp.plot(cmap=plt.cm.Blues)
            plt.title(f'Confusion Matrix - Bệnh nhân {patient_name}')
            # Lưu file vào thư mục models_output
            plt.savefig(os.path.join(model_root_path, f'{patient_name}_confusion_matrix.png'))
            plt.show() # Hiển thị trong Colab

            # 3. Vẽ ĐƯỜNG CONG ROC (ROC Curve)
            log.logger.info(f'Đang vẽ Đường cong ROC cho {patient_name}...')

            fpr, tpr, thresholds = roc_curve(y_true, y_scores)
            roc_auc = auc(fpr, tpr) # Tính lại AUC (khớp với số đã in)
            ...
            plt.title(f'ROC Curve - Bệnh nhân {patient_name}')
            plt.legend(loc="lower right")
            # Lưu file vào thư mục models_output
            plt.savefig(os.path.join(model_root_path, f'{patient_name}_roc_curve.png'))
            plt.show() # Hiển thị trong Colab

            # === KẾT THÚC CODE VẼ ĐỒ THỊ ===
...
def run_kfold_test(best_pretrain_model_path):
    ...
    best_model = torch.load(best_pretrain_model_path, weights_only=False)
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(cuda_id):
        ...
        best_pretrain_model_path = '/content/drive/MyDrive/EESNN_Project/models_output/resnet_scale0_epoch49of50_11031716.pkl'
